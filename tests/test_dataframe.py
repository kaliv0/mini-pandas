...
class TestDataFrameCreation:
    def test_input_types(self):
        ...
        with pytest.raises(ValueError):
            pdc.DataFrame({"a": np.array([1]), "b": np.array([[1]])})
    ...
    def test_array_length(self):
        with pytest.raises(ValueError):
            pdc.DataFrame({"a": np.array([1, 2]), "b": np.array([1])})
        # correct construction. no error
        pdc.DataFrame({"a": np.array([1, 2]), "b": np.array([5, 10])})
...
class TestSelection:
    ...
    def test_simple_boolean(self):
        ...
        with pytest.raises(TypeError):
            df_bool = pdc.DataFrame({"col": np.array[1, 2, 3]})  # type: ignore
